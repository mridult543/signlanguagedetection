...
async function onSubmit() {
  ...
  localStorage.setItem('my_model',JSON.stringify(content));
} 

/*async function onSubmitWt() {
  const file = document.querySelector('#select-weight').files[0];

  const content = await readFile(file);
  console.log(content);
  localStorage.setItem('my_model_weights',JSON.stringify(content));
}*/
async function train() {
  dataset.ys = null;
  dataset.encodeLabels(10);
  /*const modelFromLocalStorage = localStorage.getItem('my_model');
  const JSONSavedModel = JSON.parse(modelFromLocalStorage)
  model = await tf.loadLayersModel(JSONSavedModel);*/
  model = await tf.loadLayersModel('http://localhost:5500/my_model.json');
